    ...
    <div>
        <div v-if="tableData.length > 0">
            ...
            <div class="text-right mb-2">
                <data-download
                    :data="tissueData"
                    :filename="`tissue_expression_${tissue}`"
                ></data-download>
            </div>
        </div>
        <b-table
    ...
    data() {
        return {
            ...
            newTableFields: [
                ...
                { key: "show_datasets", label: "Datasets" },
            ],
        };
    },
    computed: {
            ...
            data.forEach((entry) => {
                if (typeof entry.tpmForAllSamples === "string") {
                    let tpms = entry.tpmForAllSamples
                        .split(",")
                        .map((i) =>
                            !!Number.isNaN(parseFloat(i)) ? 0 : parseFloat(i)
                        );
                ...
                entry["tpmLogs"] = entry.tpmForAllSamples.map((tpm) =>
                    Math.log10(tpm + 1)
                );
            });
            return data;
