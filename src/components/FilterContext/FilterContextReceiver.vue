<template>
    <FilterConsumer>
        <Emitter slot-scope="filter" :name="filter.name" :watchable="filter" :apply="apply" @change="$emit(`change${!!$event.name ? `-${$event.name}` : ''}`, $event.value, $event.name)">
            <slot><span></span></slot>
        </Emitter>
    </FilterConsumer>
</template>
