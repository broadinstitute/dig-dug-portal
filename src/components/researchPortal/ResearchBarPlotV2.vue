<template>
  ...
  export default Vue.component('research-bar-plot-v2', {
    props: {
      data: {                           //tabular data as array or object
        type: (Array, Object, null),
        required: true,
      },
      categoryKey: {                    //column key/label for the main category
        type: String,
        required: true,
      },
      totalKey: {                       //column key/label for value to use
        type: String,
        required: true,
      },
      subCategoryKeys: {                //array of column keys/labels for sub categories
        type: Array,
        required: false,
      },
      colors: {                         //array of color hex strings parallel to sub categories list
        type: Array,
        required: false,
      },
      ...
      showBarLabels:{                   //show/hide individual bar labels
        type: Boolean,
        default: true,
      }
      //axisLabels: 
    ...
    methods: {
        ...
        drawChart() {
            const { categoryKey, totalKey, subCategoryKeys, colors, normalize, barType, barWidth, showBarLabels, orientation } = this;
                    ...
                    if(this.orientation==='vertical'){
                        ...
                        singleCategoryKeys.forEach((key, i) => {
                            ...
                            if(this.showValues){
                                ...
                                .text(processedData[i][totalKey].toLocaleString())
                                .attr('fill', 'black')
                                .attr('font-size', '12px');
                            }
                        });
