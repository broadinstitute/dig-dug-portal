...
Technical overview of **GP2 Browser** in dig-dug-portal. For UI conventions and copy/persistence rules, see [`DESIGN.md`](./DESIGN.md).

---

## Overview

GP2 Browser is a browser-based genomic locus workspace. Users search a phenotype + ancestry + region, explore associations (and LD) on a shared plot stack, open section drawers for credible sets / global enrichment / V2G / S2G / genes, map variants across evidence layers, and persist work via session JSON or an HTML report.

The root shell is **`GP2Browser.vue`** (registered as `GP2Browser` in `ResearchSectionComponents.vue`). It owns session and data-layer state and composes ~41 Vue components plus ~51 JavaScript modules under this directory.

```mermaid
flowchart TB
    subgraph shell ["GP2Browser.vue"]
...
| Category | Files | Role |
|----------|-------|------|
| **Shell** | `../GP2Browser.vue` | Search orchestration, fetches, tokens, UI chrome state |
...
`GP2Browser.vue` is the single source of truth. Important groups:
...
sequenceDiagram
    participant User
    participant Shell as GP2Browser.vue
...
`GP2Browser.vue` is registered in `ResearchSectionComponents.vue` as `GP2Browser`. Portal section JSON uses `"component": "GP2Browser"`. A standalone page is also available at `/gp2browser.html`.
