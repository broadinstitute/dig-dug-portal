...
This document captures UI and product conventions for **GP2 Browser** in dig-dug-portal. Use it when adding or changing files under `gp2Browser/` and `GP2Browser.vue`.
...
- **Single persistence concept for full work:** session JSON (`app: gp2-browser`, schema v11). Recent searches are a lightweight localStorage list (≤5), not full sessions.
- **Parent-owned state.** `GP2Browser.vue` owns search session and all data layers; children emit intents and receive props.

### Top bar (left → right)

| Area | Role |
|------|------|
| **Brand** | GP2 mark + “Browser” title |
...
| Path | Purpose |
|------|---------|
| `../GP2Browser.vue` | Shell: header, canvas, drawers, session, fetches, assistant |
...
Registration: `ResearchSectionComponents.vue` case `'GP2Browser'`. Portal section JSON uses `"component": "GP2Browser"`. Standalone page: `/gp2browser.html`.
