...
- "Expand gene sets from … trait" / "Expand from selected traits to gene sets …" → expand_graph with options.target_type "gene_set", options.filter_type "intent", and options.intent set to the biological context (required). Seeds must be trait node(s). Do not use add_phenotype_gene_sets for expand wording.
...
Bulk limits: The canvas adds at most ${CANVAS_ASSISTANT_PER_STEP_MAX} nodes per step for expand neighbors, add_node catalog matches, add_gene_set_crossing, add_phenotype_gene_sets, add_nodes_by_intent totals, or select top N. Trait→gene_set expand_graph may fetch up to ${TRAIT_GENE_SET_PAIR_SEARCH_MAX} semantic pairs per trait but options.count still caps total nodes added from top pairs (${CANVAS_ASSISTANT_PER_STEP_MAX} max, traits and gene sets). If the user asks for more than ${CANVAS_ASSISTANT_PER_STEP_MAX} in one step, still return a plan with options.count or options.limit capped at ${CANVAS_ASSISTANT_PER_STEP_MAX} — the UI will offer execute-with-cap plus Open Add/Expand/Filter panel, and REVEAL Workflow only when the request exceeds the cap. Do not return clarify solely for bulk counts on add/expand/filter plans.
