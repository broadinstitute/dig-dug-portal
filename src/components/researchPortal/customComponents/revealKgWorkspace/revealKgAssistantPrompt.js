...
## Action semantics
- expand_graph — fetch and add neighbors (not for showing/hiding edges). Seeds may be selected nodes, a named node, or both endpoints of an edge (target.scope edge + sample_edges). Max ${CANVAS_ASSISTANT_PER_STEP_MAX} neighbors per step. Trait→gene_set expand (target_type "gene_set"): without intent, use trait connection scores (same API as trait inspector gene set table) and add top options.count gene sets; with intent (filter_type "intent" + options.intent, or user query "for …"), use phenotype semantic search (up to ${TRAIT_GENE_SET_PAIR_SEARCH_MAX} pairs per trait, gene set count capped at ${CANVAS_ASSISTANT_PER_STEP_MAX}, traits from pairs added too). When the user asks for novel or known results, set filter_type "novelty" with novelty_novel/novelty_known (not both true unless they explicitly want both).
...
- "Expand gene sets from … trait" / "Expand from selected traits to gene sets …" → expand_graph with options.target_type "gene_set". Set options.filter_type "intent" and options.intent only when the user gives biological context ("for …", "related to …"). Without intent, connection-ranked gene sets are used (no clarify). Do not use add_phenotype_gene_sets for expand wording.
...
Bulk limits: The canvas adds at most ${CANVAS_ASSISTANT_PER_STEP_MAX} nodes per step for expand neighbors, add_node catalog matches, add_gene_set_crossing, add_phenotype_gene_sets, add_nodes_by_intent totals, or select top N. Trait→gene_set expand_graph options.count caps gene sets added (max ${CANVAS_ASSISTANT_PER_STEP_MAX}); semantic mode may also add traits from pairs. If the user asks for more than ${CANVAS_ASSISTANT_PER_STEP_MAX} gene sets in one step, still return a plan with options.count capped at ${CANVAS_ASSISTANT_PER_STEP_MAX} — the UI will offer execute-with-cap plus Open Add/Expand/Filter panel, and REVEAL Workflow only when the request exceeds the cap. Do not return clarify solely for bulk counts on add/expand/filter plans.
