/**
 * Semantic Factor search (pgvector API) — same service biomarkerNetwork.vue uses
 * (biomarkerFactorSearch.js), reimplemented here so SCOPE's product tree stays
 * independent of that sibling component.
 *
 * Resolves free-text mechanism/outcome descriptions (e.g. "IFN-gamma secretion") to
 * real CFDE REVEAL KG Factor IRIs via semantic similarity — a plain CONTAINS match on
 * Trait labels can't do this, since Module A's outcome text rarely matches a trait
 * label verbatim (traits are diseases/phenotypes; outcomes are often mechanism-level
...
export async function searchBiomarkerFactors(needle, opts = {}) {
    ...
    const topK = Math.max(1, Math.min(50, Number(opts.limit) || 5));
    ...
    if (!response.ok) {
        throw new Error(`Biomarker factor search failed (${response.status})`);
    }
    ...
    return results
        .map((row) => {
            ...
            return {
                ...
                score:
                    row && row.score != null && !Number.isNaN(Number(row.score))
                        ? Number(row.score)
                        : null,
            };
        })
