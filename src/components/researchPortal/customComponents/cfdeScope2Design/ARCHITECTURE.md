...
**Status:** early build. Config form + SCOPE handoff + review accordion + Bedrock protocol generation are in place. **Session** menu (Reset / Import / Export) is wired via `design-session-v0` JSON. Actions / Help menus and Path A are still not wired.

**LLM provider (locked decision, 2026-09-08):** any LLM call this component makes must use `createLLMClient({ llm: "bedrock", ... })` — matching SCOPE's own default, not `cfdeDesign.vue`'s `sectionConfigs.llm || 'gemini'` default. Protocol generation (`designExperimentGenerate.js`) already follows this.
...
- The experiment-protocol JSON schema and its LLM system prompt (`experiment_system_prompt`, `cfdeDesign.vue:1112-1191`) — **done, 2026-09-09:** copied into `designExperimentGenerate.js` (`EXPERIMENT_SYSTEM_PROMPT` + `generateExperimentPlan()`), called with Bedrock.
...
- The experiment-card results rendering and `.txt` download pattern (`downloadExperiment`/`formatExperimentForDownload`, `cfdeDesign.vue:2642-2859`) — **done, 2026-09-09:** results live in `DesignExperimentResults.vue`; download text in `formatExperimentForDownload()`.
...
| Path | Purpose | Status |
|------|---------|--------|
| `../cfdeScope2Design.vue` | Shell: header, folder tabs, config panel, review accordion, Generate → Bedrock, Session menu | Built |
| `cfdeScope2Design/DesignMenuBar.vue` | Session / Actions / Help top menus | Built — Session wired; Actions/Help still stubbed |
| `cfdeScope2Design/designExperimentParams.js` | `ASSAY_TYPES`/`CELL_TYPES`/`ASSAY_READOUTS` reference data | Built |
| `cfdeScope2Design/designExperimentGenerate.js` | System prompt, user prompt builder, JSON extract, Bedrock generate, download formatter | Built |
| `cfdeScope2Design/DesignExperimentResults.vue` | Protocol cards + disclaimer/citation/download UI (from `cfdeDesign.vue`) | Built |
| `cfdeScope2Design/designSessionFile.js` | `design-session-v0` export/import (save via File System Access API or download fallback) | Built |
...
| 2026-09-09 | Review step: `reviewAndGenerate()` opens the Experiment protocol tab with a collapsible Configuration Summary accordion recycled from `cfdeDesign.vue`'s `#planner-search-draft` / `.search-summary` / `.config-grid` card (hypothesis, genes, experiment constraints, optional assay/cell/readout selections, and throughput/species/timeline). Skipped the multi-gene strategy radios — DESIGN is single-hypothesis. "Generate Experiment" collapses the accordion (`reviewAccordionOpen = false`); LLM protocol generation still not wired |
| 2026-09-09 | Protocol generation: `designExperimentGenerate.js` recycles `cfdeDesign.vue`'s system prompt + single-call generate path + download formatter, forced to Bedrock. `DesignExperimentResults.vue` recycles the experiment-card / disclaimer / citation / download UI. `generateExperiment()` collapses the review accordion, shows a loading state, then renders `resultModel` cards (no per-gene sequential or per-group strategies — out of scope for SCOPE-fed DESIGN) |
| 2026-09-09 | Session menu wired: Reset clears config + protocol results; Export/Import use `designSessionFile.js` (`design-session-v0` JSON capturing form fields, advanced params, and `experiment_results`). Same File System Access API / download-fallback pattern as SCOPE's `scopeSessionFile.js`, kept in DESIGN's own tree |
