    ...
    methods: {
        ...
        getTableData() {
            ...
            keyFieldVals.forEach((item) => {
                let filteredDatasets = processedData.filter(
                    (entry) => entry[this.keyField] === item
                );
                let tpms = filteredDatasets
                    .reduce(
                        (list, entry) => list.concat(entry.tpmForAllSamples),
                        []
                    )
                    .sort(d3.ascending);
                let singleRow = {
