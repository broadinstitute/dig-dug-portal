            ...
            <div class="glens-phenotype" :class="{ 'glens-large-text': largeText }">
                    ...
                    <div class="glens-hero-results">
                        ...
                        <div class="glens-summary-group glens-summary-group--triplet">
                            <div class="glens-summary-cell">
                                <span>{{ annotationBurdenSummary.label }}</span>
                                <strong>{{ annotationBurdenSummary.value }}</strong>
                                <small>{{ annotationBurdenSummary.detail }}</small>
                            </div>
                            <div class="glens-summary-cell">
                ...
                <section class="glens-insight-tabs-card">
                    ...
                    <section v-if="activeInsightTab === 'gene'" class="glens-step-card glens-step-card--tabbed">
                        <div class="glens-step-body">
                            <p class="glens-source-note">
                                Gene candidates are derived from profile-matched disease/gene annotations, then checked for CRDC recurrence in phenotype-matched samples.
                            </p>
                            <div class="glens-candidate-table glens-candidate-table--gene">
                                ...
                                <template v-for="row in phenotype.geneCandidates">
                                    <div
                                        :key="`${row.gene}-row`"
                                        class="glens-candidate-row"
                                    >
                                        ...
                                        <span>
                                            <button
                                                class="glens-inline-link glens-inline-link--button"
                                                type="button"
                                                @click.stop="toggleReferenceDetail(`gene-profile-${row.gene}`)"
                                            >
                                                Query phenotype overlap
                                            </button>
                                        </span>
                                        <span>
                                            <button
                                            ...
                                            >
                                                {{ phenotype.queryTerms.exact.length }} / {{ phenotype.queryTerms.exact.length }} query HPO terms
                                            </button>
                                        </span>
                                        <span>{{ normalizedExternalAnnotation(row.externalAnnotation) }}</span>
                                        <span>{{ readableCarrierEvidence(row) }}</span>
                                    </div>
                                    <div
                                    ...
                                    >
                                        ...
                                        <p v-if="referenceDetailNote(row)">{{ referenceDetailNote(row) }}</p>
                                        <div
                                            v-if="activeReferenceDetail.includes('-profile-') || activeReferenceDetail.includes('-terms-')"
                                            class="glens-accordion-table glens-accordion-table--terms"
                                        >
                                            <div>
                                                <span>Matched</span>
                                                <span>HPO term</span>
...
import "../KrPhenotype/style.css";

export default {
    name: "KrPhenotypeNewTemplate",
    components: {
        ClinicalFocusBar,
    },
    data() {
    ...
    computed: {
        ...
        cohortSummary() {
            ...
            return {
                ...
                sex: `${summaryUsesAllMatched ? "Sex" : "Sex in displayed rows"}: ${sexParts.join(" · ") || "not available"}`,
                proband: `${summaryUsesAllMatched ? "Proband status" : "Proband status in displayed rows"}: ${probandParts.join(" · ") || "not available"}`,
            };
        },
    ...
    methods: {
        ...
        referenceGeneTermRows(row) {
            return this.referenceMatchedTerms(row).map((term) => ({
                ...this.termObject(term, "Query/co-observed phenotype support"),
                matched: true,
            }));
        },
        ...
        referenceDetailNote(row) {
            if (this.activeReferenceDetail.includes("-crdc-")) {
                return row.gene
                    ? this.readableCarrierEvidence(row)
            ...
            if (row.gene) {
                return "These HPO terms are the searched phenotype profile terms and the strongest co-observed CRDC phenotype signals currently available in the fixture.";
            }
        ...
        phenotypeTermHref(term) {
            return `/krPhenotype_new.html?term=${encodeURIComponent(term.id)}&label=${encodeURIComponent(term.label)}`;
        },
        sampleHref(sampleId) {
...
.glens-candidate-table--gene .glens-candidate-row {
    grid-template-columns: minmax(7rem, 0.55fr) minmax(11rem, 0.9fr) minmax(12rem, 0.95fr) minmax(14rem, 1.1fr) minmax(17rem, 1.3fr);
    min-width: 72rem;
}
