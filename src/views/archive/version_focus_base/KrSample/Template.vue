        ...
        <div class="container-fluid mdkp-body glens-page">
            <main class="glens-sample" :class="{ 'glens-large-text': largeText }">
                ...
                <section class="glens-tab-card">
                    ...
                    <section v-if="activeTab === 'phenotype'" class="glens-tab-panel">
                        ...
                        <div v-if="openPanels.phenotypeProfile" class="glens-accordion-body">
                            <div class="glens-domain-list">
                                <div v-for="domain in sample.phenotypeDomains" :key="domain.name" class="glens-domain-row">
                                    <div class="glens-domain-summary">
                                        <strong>{{ domain.name }}</strong>
                                        <span>{{ domain.count }} / {{ sample.hpoTotal }} HPO terms</span>
                                    </div>
                                    <div class="glens-bar-shell">
                                        <div class="glens-bar" :style="{ width: domain.width }"></div>
                                    </div>
                                    <div class="glens-term-grid">
                    ...
                    <section v-if="activeTab === 'compare'" class="glens-tab-panel">
                        <div class="glens-section-head">
                            <div>
                                <p class="glens-eyebrow">Compare with reference patient</p>
                                <h2>Use only when a known reference case should be checked 1:1</h2>
                            </div>
                        </div>

                        <div class="glens-compare-card">
                            <div class="glens-compare-inputs">
                                <label>
                                    Current sample
                                    <input :value="displaySampleId" readonly />
                                </label>
                                <label>
                                    Reference sample
                                    <input v-model="referenceSampleId" placeholder="BCH-12-34210-01" />
                                </label>
                                <button type="button">Compare</button>
                            </div>
                            <div class="glens-compare-grid">
                                <div v-for="item in sample.comparePreview" :key="item.label">
                                    <span>{{ item.label }}</span>
                                    <strong>{{ item.value }}</strong>
                                    <p>{{ item.text }}</p>
                                </div>
