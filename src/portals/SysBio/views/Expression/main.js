...
new Vue({
    ...
    watch: {
        pValThreshold() {
            this.bulkVolcanoConfig["y condition"]["greater than"] =
                this.pValThreshold;
        },
    },
    ...
    computed: {
        ...
        tableData() {
            ...
            if (this.showGenes) {
                data = data.filter(
                    (item) => this.showRegulation(item) === this.showGenes
                );
            }
            return data;
        },
    ...
    methods: {
        ...
        goToHighlightedRow(gene) {
            ...
            const index = this.filteredRows.findIndex(
                (item) => item.GENE === this.highlightGene
            );
            if (index !== -1) {
        ...
        rowClasses(item) {
            let classString = this.isHighlightedGene(item).concat(
                this.showRegulation(item)
            );
            return classString;
        },
        ...
        showRegulation(item) {
            if (
                item[this.bulkVolcanoConfig["y axis field"]] <
                this.bulkVolcanoConfig["y condition"]["greater than"]
            ) {
                return "";
            }
            if (
                item[this.bulkVolcanoConfig["x axis field"]] <=
                this.bulkVolcanoConfig["x condition"]["lower than"]
            ) {
                return this.down;
            }
            if (
                item[this.bulkVolcanoConfig["x axis field"]] >=
                this.bulkVolcanoConfig["x condition"]["greater than"]
            ) {
                return this.up;
            }
            return "";
