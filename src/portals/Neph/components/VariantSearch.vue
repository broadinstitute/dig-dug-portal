			...
			if (this.variants && this.variants.length) {
				for (let i = 0; i < this.variants.length; i++) {
					...
					if (this.variants[i].veprecords.length > 0) {
						...
						//console.log(JSON.stringify(varrecords));
						
						for (let j = 0; j < varrecords.length; j++) {
							if(varrecords[j].HGVSc.length > 18){
								varrecords[j].HGVSc = varrecords[j].HGVSc.substring(18);
							}
							if(varrecords[j].HGVSp.length > 18){
								varrecords[j].HGVSp = varrecords[j].HGVSp.substring(18);
								varrecords[j].HGVSp = varrecords[j].HGVSp.replace("%3D","=")
							}
