                                ...
                                <div v-if="donorAvailabilityPlotData.length && activeContentTab === 'plot'" class="dataset-content-panel" role="tabpanel">
                                    <div class="availability-plot-card">
                                        <div class="availability-plot-summary">
                                            <div class="availability-plot-summary-count">
                                            ...
                                            <button
                                                class="availability-plot-summary-link"
                                                type="button"
                                                @click="showAssayDonorsInTable"
                                            >
                                                See table
                                            </button>
                                        </div>
                                            ...
                                            <div class="availability-plot-panel">
                                                ...
                                                <div class="availability-plot-panel-toolbar">
                                                    <div class="availability-plot-controls">
                                                        ...
                                                        >
                                                            <option :value="NO_PLOT_GROUPING">None</option>
                                                            <option
                                            ...
                                            <div class="availability-data-type-grid">
                                                ...
                                                >
                                                    ...
                                                    <div class="availability-data-type-metric">
                                                        <div class="availability-data-type-metric-count">
                                                            <span class="availability-data-type-metric-value">{{ item.donorCount.toLocaleString() }}</span>
                                                            <span class="availability-data-type-metric-label">Donors</span>
                                                        </div>
                                                        <button
                                ...
                                <div v-if="activeContentTab === 'table'" class="dataset-content-panel" role="tabpanel">
                                    <div class="dataset-toolbar">
                                        ...
                                        <div class="dataset-toolbar-actions">
                                            ...
                                            <data-download
                                                :data="mainTableDownloadRows"
                                                :filename="mainTableDownloadFilename"
                                            ></data-download>
                                            <table-config-button
                                ...
                                >
                                    ...
                                    <div class="dataset-toolbar">
                                        ...
                                        <div class="dataset-toolbar-actions">
                                            ...
                                            <data-download
                                                :data="getScopedTableDownloadRows(tab)"
                                                :filename="getScopedTableDownloadFilename(tab)"
                                            ></data-download>
                                            <table-config-button
                                                v-if="showConfigureControls"
                                                :columns="availableColumns"
...
import { formatDataTypeLabel, getDataTypeDetails, normalizeDataTypeValue } from "./dataTypeUtils";
...
const NO_PLOT_GROUPING = "__none__";
...
export default {
    ...
    data() {
        return {
            ...
            availabilityPlotChartId: "pankbase-donors-availability-plot",
        };
    },
    computed: {
        preparedDataset() {
        ...
        isPlotUngrouped() {
            return this.plotCategoryField === this.NO_PLOT_GROUPING;
        },
        resolvedPlotCategoryField() {
            if (this.isPlotUngrouped) {
                return "";
            }
        ...
        donorAvailabilityOverlapRows() {
            return Object.keys(this.donorAvailabilityAssayTypeTotals)
                ...
                .map((dataType) => ({
                    ...
                    count: this.donorAvailabilityAssayTypeTotals[dataType] || 0,
                }));
        },
        ...
        mainTableDownloadRows() {
            return this.buildTableRows(this.filteredRows);
        },
        mainTableDownloadFilename() {
    ...
    watch: {
        ...
        availabilityDataTypeCards(cards) {
            ...
            validScopes.add("scope:assay-donors");
            cards.forEach((card) => {
            ...
            const getTabScopeKey = (tab) => {
                if (tab.scopeType === "assay-donors") {
                    return "scope:assay-donors";
                }

                return `${tab.dataType}|||${tab.groupValue || ""}`;
            };
            const removedTabs = this.openTableTabs.filter((tab) => !validScopes.has(getTabScopeKey(tab)));
            const remainingTabs = this.openTableTabs.filter((tab) => validScopes.has(getTabScopeKey(tab)));
    ...
    methods: {
        ...
        showAssayDonorsInTable(event = null) {
            const trigger = event && event.currentTarget ? event.currentTarget : null;
            if (trigger && typeof trigger.blur === "function") {
                trigger.blur();
            }

            const tabKey = "assay-donors";
            const existingTab = this.openTableTabs.find((tab) => tab.key === tabKey);
            if (existingTab) {
                this.activeContentTab = existingTab.key;
                this.scrollContentTabsIntoView();
                return;
            }
            ...
            const nextTab = {
                key: tabKey,
                scopeType: "assay-donors",
                title: "Donors with assays",
            };
        ...
        getDataTypeDetails(dataType) {
            return getDataTypeDetails(dataType);
        },
        ...
        getRowsForTableScope(tab) {
            if (tab.scopeType === "assay-donors") {
                ...
                return this.filteredRows.filter((row) => this.parseAvailabilityValues(row[availabilityKey]).length > 0);
            }
        ...
        getScopedTableDownloadRows(tab) {
            return this.buildTableRows(this.getRowsForTableScope(tab));
        },
        getScopedTableDownloadFilename(tab) {
            if (tab.scopeType === "assay-donors") {
                return "pankbase_donors_with_assays";
            }

            const scopeParts = [
                "pankbase_donors",
                this.slugifyValue(this.getDataTypeLabel(tab.dataType)),
            ];

            if (tab.groupValue) {
                scopeParts.push(this.slugifyValue(tab.groupValue));
            }

            return scopeParts.filter(Boolean).join("_");
        },
        ...
        getScopedTableFilterSummary(tab) {
            ...
            if (tab.scopeType === "assay-donors") {
                return ["Scope: Donors with assays"].concat(filterLabels).join(" | ");
            }
        ...
        getAvailabilityPlotSvgElement() {
            return document.getElementById(this.availabilityPlotChartId);
        },
        getAvailabilityPlotLegendLayout(width) {
            const items = this.donorAvailabilityLegend.map((item) => ({
                label: item.label,
            ...
            items.forEach((item) => {
                const estimatedWidth = 22 + (item.label.length * 7) + (item.count.length * 7) + 26;
                if (currentRow.length && currentWidth + estimatedWidth > maxWidth) {
            ...
            return {
                items: rows.reduce((layoutItems, row, rowIndex) => {
                    let x = 16;
                    row.forEach((item) => {
                        layoutItems.push({
                            ...item,
        ...
        buildAvailabilityPlotExportSvgMarkup() {
            const sourceSvg = this.getAvailabilityPlotSvgElement();
            if (!sourceSvg) {
                return "";
            }

            const serializer = new XMLSerializer();
            const sourceClone = sourceSvg.cloneNode(true);
            ...
            const legendGap = legendLayout.height ? 12 : 0;
            const chartOffsetY = 16 + titleBlockHeight + legendGap + legendLayout.height;
            const totalHeight = chartOffsetY + height;
            const legendMarkup = legendLayout.items.map((item) => {
                const labelX = item.x + 16;
                const countX = item.x + item.width - 10;
                return `
                    <rect x="${item.x}" y="${56 + item.y}" width="10" height="10" rx="2" fill="${item.color}"></rect>
                    <text x="${labelX}" y="${65 + item.y}" fill="#4f6571" font-size="12" font-family="'Open Sans', sans-serif">${this.escapeSvgText(item.label)}</text>
                    <text x="${countX}" y="${65 + item.y}" fill="#7d7469" font-size="12" font-weight="600" text-anchor="end" font-family="'Open Sans', sans-serif">${this.escapeSvgText(item.count)}</text>
                `;
            }).join("");

            sourceClone.setAttribute("x", "0");
            sourceClone.setAttribute("y", String(chartOffsetY));
            sourceClone.setAttribute("width", String(width));
            sourceClone.setAttribute("height", String(height));

            return `
                <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${totalHeight}" viewBox="0 0 ${width} ${totalHeight}" font-family="'Open Sans', sans-serif">
                    <rect width="${width}" height="${totalHeight}" fill="#ffffff"></rect>
                    <text x="16" y="24" fill="#22343f" font-size="16" font-weight="700" font-family="'Open Sans', sans-serif">${this.escapeSvgText(title)}</text>
                    ${subtitle ? `<text x="16" y="44" fill="#4f6571" font-size="12" font-weight="600" font-family="'Open Sans', sans-serif">${this.escapeSvgText(subtitle)}</text>` : ""}
                    ${legendMarkup}
                    ${serializer.serializeToString(sourceClone)}
                </svg>
            `;
        },
        downloadAvailabilityPlotSvg() {
            const svgMarkup = this.buildAvailabilityPlotExportSvgMarkup();
            if (!svgMarkup) {
                return;
            }
            ...
            uiUtils.downloadChart(url, `${this.availabilityPlotDownloadFilename}.svg`);
            window.setTimeout(() => {
                URL.revokeObjectURL(url);
            }, 0);
        },
        downloadAvailabilityPlotPng() {
            const svgMarkup = this.buildAvailabilityPlotExportSvgMarkup();
            if (!svgMarkup) {
                return;
            }
...
.availability-plot-summary-count {
    display: flex;
    align-items: baseline;
    gap: 10px;
}
...
.availability-plot-summary-link {
    border: 0;
    padding: 0;
    background: transparent;
    color: #229098;
...
.availability-data-type-metric-count {
    display: flex;
    align-items: baseline;
    gap: 5px;
}
