...
const datasetsTemp = {
    SingleCell_Emont2022_Humans_SCP1376: {
        study_design:
            "We collected subcutaneous WAT from nine women, isolated single cells from the SVF using collagenase digestion, and then performed whole-cell Drop-seq. Because different depots have been differentially linked to metabolic disease, for the second cohort we collected paired subcutaneous (SAT) and visceral (VAT) adipose tissue from ten individuals and SAT alone from three additional individuals (ten women and three men), and performed sNuc-seq. Doublet and low-quality filtering left 166,149 total cells (28,465 single cells and 137,684 single nuclei). The data from both approaches were integrated.",
        cell_info:
        ...
        genes_info:
            "Marker genes for each cell population in the human WAT dataset.",
        samples: "biosample_id",
        donors: "donor_id",
        study_characteristics: [
            ["fat__type", "sex"],
            ["fat__type", "library_preparation_protocol__ontology_label"],
        ],
...
new Vue({
    ...
    computed: {
        listOfCategories() {
            ...
            const filtered = allCategories.filter(
                (category) => source[category].length > 1
            );
        ...
        currentDataset() {
            ...
            return this.datasets.filter(
                (dataset) => dataset.datasetId === this.activeDataset
            )[0];
        },
    ...
    methods: {
        ...
        async init() {
            ...
            this.colorScalePlasmaColorsArray = d3
                .range(0, 1.01, 0.1)
                .map((t) => this.colorScalePlasma(t))
                .join(", ");
            ...
            const parsedData = this.parseRawDataByCategory(
                this.rawData,
                allCategories
            );
            ...
            if (!this.datasetConfig) {
                ...
                const donorsCategory = allCategories.filter((category) =>
                    category.includes("donor")
                );
                const samplesCategory = allCategories.filter((category) =>
                    category.includes("sample")
                );
                const cellClusterCategory = allCategories.filter((category) =>
                    category.includes("cluster")
                );
                const cellTypeCategory = allCategories.filter(
                    (category) =>
                        category.includes("cell") && category.includes("type")
                );
                ...
                const allCategoriesMinusExcluded = allCategories.filter(
                    (word) => !exclusionWords.includes(word)
                );
                this.datasetConfig = {
            ...
            this.datasetSettings = JSON.parse(
                JSON.stringify(this.datasetConfig)
            );
            this.datasetSettings.selectedCellType =
                this.datasetConfig.cellTypes[0];

            const parsedDataDonors = this.filterUniqueByCategory(
                parsedData,
                this.datasetConfig.donors
            );
            const parsedDataSamples = this.filterUniqueByCategory(
                parsedData,
                this.datasetConfig.samples
            );
            ...
            if (this.datasetConfig.study_characteristics) {
                const studyCategories = [
                    ...new Set([
                        this.datasetConfig.donors,
                        ...this.datasetConfig.study_characteristics.flat(),
                    ]),
                ];
                const parsedDataTest = this.parseRawDataByCategory(
                    this.rawData,
                    studyCategories
                );
                const parsedDataDonors2 = this.filterUniqueByCategory2(
                    parsedDataTest,
                    this.datasetConfig.donors
                );
                ...
                if (this.datasetConfig.study_characteristics) {
                    ...
                    this.datasetConfig.study_characteristics.forEach((item) => {
                        if (Array.isArray(item) && item.length === 2) {
                            const counts = this.getGroupedCounts(
                                parsedDataDonors2,
                                item
                            );
                            results.push({
            ...
            //donors
            const donorInfoSubset =
                this.datasetConfig.donor_characteristics.slice(0, 4);
            this.updateDonorsInfo(donorInfoSubset);

            //cell types
            this.datasetSettings.selectedConditions = [null, null];
            this.datasetSettings.selectedConditions[0] =
                this.datasetConfig.conditions[0];
            ...
            //marker genes
            if (
                this.datasetConfig.markerGenes &&
                this.datasetConfig.markerGenes.length > 0
            ) {
            ...
            if (datasetSetup) {
                ...
                const cellTypesCategory = Array.isArray(datasetSetup.cellTypes)
                    ? datasetSetup.cellTypes[0]
                    : datasetSetup.cellTypes;
                const conditionsCategory = Array.isArray(
                    datasetSetup.conditions
                )
                    ? datasetSetup.conditions[0]
                    : datasetSetup.conditions;
                ...
                const characteristics = await this.processData({
                    left: Array.isArray(datasetSetup.donors)
                        ? datasetSetup.donors
                        : [datasetSetup.donors],
                    right: datasetSetup.donor_characteristics,
                });

                this.processCharacterisitcs(
                    characteristics,
                    datasetSetup.donor_characteristics
                );
            }

            //
            await this.calculateTable({
                left: this.categoriesLeft,
                right: this.categoriesRight,
            });

            //
            const rect = document
                .querySelector(".sidebar-parent")
        ...
        async fetchFields() {
            ...
            try {
                const response = await fetch(
                    `${BIO_INDEX_HOST}/api/raw/file/single_cell/${this.activeDataset}/fields.json.gz`
                );
        ...
        async fetchCoordinates() {
            ...
            try {
                const response = await fetch(
                    `${BIO_INDEX_HOST}/api/raw/file/single_cell/${this.activeDataset}/coordinates.tsv.gz`
                );
        ...
        async fetchGeneExpression(gene) {
            ...
            try {
                const response = await fetch(
                    `${BIO_INDEX_HOST}/api/bio/query/single-cell-lognorm?q=${this.activeDataset},${gene}`
                );
                ...
                if (this.datasetSettings) {
                    const geneStats = await this.getGeneExpression(
                        this.expressionData[gene],
                        this.datasetSettings.selectedCellType
                    );
        ...
        filterUniqueByCategory2(parsedData, key) {
            ...
            return parsedData.filter((item) => {
                const keyValue = item[key];
                const restOfItem = JSON.stringify({
                    ...item,
                    [key]: undefined,
                });
        ...
        getGroupedCounts(parsedData, categoryKeys, uniqueKey = null) {
            ...
            const groupedCounts = groupAndCount(
                parsedData,
                uniqueKey ? [uniqueKey, ...categoryKeys] : categoryKeys
            );
        ...
        bTableFromGroupedCounts(groupedCountsData) {
            ...
            if (depth < 2 || depth > 3) {
                throw new Error(
                    "Object depth must be 2 or 3 for this table structure"
                );
            }
            ...
            if (depth === 2) {
                ...
                if (depth === 2) {
                    ...
                    columns.unshift({
                        key: "_rowVariant",
                        label: this.datasetSettings.selectedCellType,
                    });
                    ...
                    for (let topKey of uniqueKeys[0]) {
                        ...
                        for (let colKey of uniqueKeys[1]) {
                            const value =
                                groupedCountsData[topKey]?.[colKey] || 0;
                            row[colKey] = value;
                            rowTotal += value;
                            footerRow[colKey] =
                                (footerRow[colKey] || 0) + value;
                        }
                ...
                if (depth === 3) {
                    ...
                    for (let depth_0_key of uniqueKeys[0]) {
                        ...
                        columns.push({
                            key: `${depth_0_key}_total`,
                            label: `${depth_0_key} Total`,
                        });
                        if (
                            depth_0_key !==
                            uniqueKeys[0][uniqueKeys[0].length - 1]
                        ) {
                            columns.push({
                                key: `spacer_${depth_0_key}`,
                                label: "",
                            });
                        }
                    }
                    ...
                    for (let depth_1_key of uniqueKeys[1]) {
                        ...
                        for (let depth_0_key of uniqueKeys[0]) {
                            ...
                            for (let depth_2_key of uniqueKeys[2]) {
                                const value =
                                    groupedCountsData[depth_0_key]?.[
                                        depth_1_key
                                    ]?.[depth_2_key] || 0;
                                row[`${depth_0_key}_${depth_2_key}`] = value;
                                sectionTotal += value;
                                footerRow[`${depth_0_key}_${depth_2_key}`] =
                                    (footerRow[
                                        `${depth_0_key}_${depth_2_key}`
                                    ] || 0) + value;
                            }
                            row[`${depth_0_key}_total`] = sectionTotal;
                            footerRow[`${depth_0_key}_total`] =
                                (footerRow[`${depth_0_key}_total`] || 0) +
                                sectionTotal;
        ...
        getGeneExpression(expressionData, category) {
            ...
            categoryLabels.forEach((label) => {
                const sortedValues = geneExpression[label]
                    ? geneExpression[label].sort(d3.descending)
                    : [0];
                const key = label;
                ...
                const pctExpr =
                    (sortedValues.filter((val) => val > 0).length /
                        sortedValues.length) *
                    100; //sortedValues.length / 166149;
                sumstat.push({
                    key,
                    mean,
                    q1,
        ...
        getGeneExpressionCombined(expressionData, categoryA, categoryB) {
            ...
            for (var i = 0; i < expressionData.length; i++) {
                ...
                if (!geneExpression[labelA][labelB])
                    geneExpression[labelA][labelB] = [];
        ...
        calcCombinedExpressionStats(gene) {
            ...
            const statsA = this.getGeneExpression(
                expressionData,
                this.datasetSettings.selectedCellType
            );
            const statsB = this.getGeneExpression(
                expressionData,
                this.datasetSettings.selectedConditions[0]
            );
            const combinedStats = this.getGeneExpressionCombined(
                expressionData,
                this.datasetSettings.selectedCellType,
                this.datasetSettings.selectedConditions[0]
            );
        ...
        colorsByCategory(category) {
            ...
            const sortedColors = Object.values(
                this.sortObjectKeysLocale(categoryColors)
            );
        ...
        updateDonorsInfo(categories) {
            const parsedData = this.parseRawDataByCategory(this.rawData, [
                this.datasetConfig.donors,
                ...categories,
            ]);
            console.log("     parsedData", parsedData);
            const filterUniqueDonors = this.filterUniqueByCategory(
                parsedData,
                this.datasetConfig.donors
            );
            const countsByDonor = this.getCounts(
                filterUniqueDonors,
                categories
            );
        ...
        async updateCellsInfo(cellTypeCategory, conditionA, conditionB) {
            ...
            const bTableFromCounts = this.bTableFromCounts(
                counts,
                cellTypeCategory
            );
            ...
            const d = (label, key, subKey, value, subKeySubset = null) => {
                return {
                    ...
                    subKeyFields: [],
                    data: value,
                    umapColors: this.calcUmapColorsByCategory(
                        subKey,
                        subKeySubset
                    ),
                };
            ...
            const test = this.getGroupedCounts(
                parsedData,
                [cellTypeCategory, conditionA],
                conditionB
            );
            ...
            if (conditionB) {
                for (const [key, value] of Object.entries(test)) {
                    ...
                    cellTypeConditions.push(
                        d(key, cellTypeCategory, conditionA, value)
                    );
                }
            } else {
                cellTypeConditions.push(
                    d("", cellTypeCategory, conditionA, test)
                );
            }
        ...
        toggleCategorySelect(e) {
            ...
            if (selectParent === "select-donor-categories") {
                if (this.donorInfo[selected]) {
                    ...
                    this.updateDonorsInfo([
                        ...Object.keys(this.donorInfo),
                        selected,
                    ]);
                }
            }
            ...
            if (selectParent === "select-cell-types") {
                if (this.datasetSettings.selectedCellType !== selected) {
                    ...
                    this.updateCellsInfo(
                        selected,
                        this.datasetSettings.selectedConditions[0],
                        this.datasetSettings.selectedConditions[1]
                    );
                    if (this.activeGene) {
                        this.expressionStats = [];
                        this.geneNames.forEach((gene) => {
                            const geneStats = this.getGeneExpression(
                                this.expressionData[gene],
                                this.datasetSettings.selectedCellType
                            );
                            this.expressionStats.push({ [gene]: geneStats });
            ...
            if (selectParent === "select-conditions") {
                ...
                if (side === "b") {
                    this.datasetSettings.selectedConditions[1] =
                        this.datasetSettings.selectedConditions[1] === selected
                            ? null
                            : selected;
                }
        ...
        calcFieldColors(rawData) {
            const colors = {};
            for (const [key, value] of Object.entries(
                rawData["metadata_labels"]
            )) {
                colors[key] = {};
                for (var i = 0; i < value.length; i++) {
                    colors[key][value[i]] = this.colorScaleIndex(
                        this.colorIndex
                    );
                    this.colorIndex++;
                }
        ...
        async processData(categories) {
            const allUserSelectedCategories = categories.left.concat(
                categories.right
            );
            console.log(
                "allUserSelectedCategories",
                allUserSelectedCategories,
                typeof allUserSelectedCategories
            );
        ...
        processCellCounts(processedData, categories) {
            ...
            const leftIsNew = !this.areArraysEqual(
                categories.left,
                this.lastCategoriesLeft
            );
            const rightIsNew = !this.areArraysEqual(
                categories.right,
                this.lastCategoriesRight
            );
            ...
            if (categories.left.length > 0) {
                ...
                const aFreq = this.frequencyDistribution(
                    processedData,
                    categories.left
                );
                const aTable = this.frequencyTable(aFreq, categories.left);
                this.aRows = aTable.rows;
                this.sortedRowsA = this.localSort(
                    this.aRows,
                    this.categoriesLeft.join("|"),
                    false
                );
            ...
            if (categories.right.length > 0) {
                ...
                const bFreq = this.frequencyDistribution(
                    processedData,
                    categories.right
                );
                const bTable = this.frequencyTable(bFreq, categories.right);
                this.bRows = bTable.rows;
                this.sortedRowsB = this.localSort(
                    this.bRows,
                    this.categoriesRight.join("|"),
                    false
                );
            ...
            if (categories.left.length > 0 && categories.right.length > 0) {
                ...
                const crossTabData = this.crossTabulation(
                    processedData,
                    categories.left,
                    categories.right
                );
            ...
            for (let i = 0; i < NAME.length; i++) {
                if (hasLeft && leftIsNew) {
                    ...
                    const label1 =
                        metadata_labels[categories.left[0]][labelIdx1];
                    if (categoryLeftCount < 2) {
                        pointColors.left[i] =
                            this.fieldColors[categoryLeft][`${label1}`];
                    } else {
                        const labelIdx2 = metadata[categories.left[1]][i];
                        const label2 =
                            metadata_labels[categories.left[1]][labelIdx2];
                        pointColors.left[i] =
                            this.fieldColors[categoryLeft][
                                `${label1}|${label2}`
                            ];
                    }
                }
                if (hasRight && rightIsNew) {
                    ...
                    const label1 =
                        metadata_labels[categories.right[0]][labelIdx1];
                    if (categoryRightCount < 2) {
                        pointColors.right[i] =
                            this.fieldColors[categoryRight][`${label1}`];
                    } else {
                        const labelIdx2 = metadata[categories.right[1]][i];
                        const label2 =
                            metadata_labels[categories.right[1]][labelIdx2];
                        pointColors.right[i] =
                            this.fieldColors[categoryRight][
                                `${label1}|${label2}`
                            ];
                    }
                }
        ...
        frequencyDistribution(data, category) {
            ...
            const result = data.reduce((acc, record) => {
                ...
                const label = !isMulti
                    ? record[category]
                    : `${record[category[0]]}|${record[category[1]]}`;
            ...
            if (isMulti) {
                ...
                if (!this.fieldColors[combinedCategoryLabel]) {
                    const combinedColors = this.calcCombinedFieldColors(
                    ...
                    Vue.set(
                        this.fieldColors,
                        combinedCategoryLabel,
                        combinedColors[combinedCategoryLabel]
                    );
                }
        ...
        crossTabulation(data, categoryA, categoryB) {
            ...
            return data.reduce((acc, record) => {
                ...
                const label1 = !isMultiA
                    ? record[categoryA]
                    : `${record[categoryA[0]]}|${record[categoryA[1]]}`;
                const label2 = !isMultiB
                    ? record[categoryB]
                    : `${record[categoryB[0]]}|${record[categoryB[1]]}`;
        ...
        frequencyCrossTable(data) {
            ...
            //sort column keys
            const allColumnKeysSorted = Array.from(allColumnKeys).sort((a, b) =>
                a.localeCompare(b)
            );
            ...
            headers2.push({
                label: this.categoriesRight.join("|"),
                colspan: allColumnKeys.size,
            });
            ...
            this.rows = rows;
            this.sortedRows = this.localSort(
                rows,
                this.categoriesLeft.join("|"),
                false
            );
            ...
            console.log("   chi2");
            const rowKeys = this.sortedRows.map(
                (row) => row[this.categoriesLeft.join("|")]
            );
            const columnKeys = [];
            const groups = [];
            ...
            console.log(
                "      pValues after Bonferroni correction",
                correctedPValues
            );

            function fdrCorrection(pValues, alpha = 0.01) {
                const sortedPValues = [...pValues].sort(
        ...
        calcCombinedFieldColors(comboCategory, fields) {
            ...
            for (var i = 0; i < fields.length; i++) {
                colors[comboCategory][fields[i]] = this.colorScaleIndex(
                    this.colorIndex
                );
                this.colorIndex++;
            }
        ...
        categoryKeys(category, side) {
            ...
            console.log("categoryKeys", {
                ...
                side,
                isMulti,
                combos: this.categoryCombos,
                keys: this.rawData["metadata_labels"][category],
            });
            if (isMulti) {
                return this.categoryCombos[category.join("|")];
            } else {
                if (side === "right")
                    return this.sortedRowsB.map((item) => item[category]);
        ...
        categoryColors(side) {
            ...
            if (side === "right")
                return this.sortedRowsB.map(
                    (item) => this.fieldColors[category][item[category]]
                );
            return this.sortedRowsA.map(
                (item) => this.fieldColors[category][item[category]]
            );
        },
        ...
        parseGeneExpression() {
            ...
            const expressionByCategory = (category) => {
                ...
                this.geneNames.forEach((gene) => {
                    ...
                    categoryData.forEach((labelIdx, cellIdx) => {
                        ...
                        geneExpression[gene][label].push(
                            this.expressionData[gene][cellIdx]
                        );
                    });
                    geneExpression[gene] = this.sortObjectKeysLocale(
                        geneExpression[gene]
                    );

                    categoryLabels.forEach((label) => {
                        const sortedValues = geneExpression[gene][label]
                            ? geneExpression[gene][label].sort(d3.descending)
                            : [0];
                        const key = label;
                        const mean = d3.mean(sortedValues);
                        const q1 = d3.quantile(sortedValues, 0.25);
            ...
            if (this.categoriesLeft.length === 1) {
                this.geneExpressionA = expressionByCategory(
                    this.categoriesLeft
                );
            ...
            if (this.categoriesRight.length === 1) {
                this.geneExpressionB = expressionByCategory(
                    this.categoriesRight
                );
        ...
        getCombinedGeneExpression(gene) {
            ...
            if (this.categoriesLeft === 0 || this.categoriesRight.length === 0)
                return;
            ...
            const expressionCombined = (gene) => {
                const categoryLabelsA =
                    this.rawData["metadata_labels"][this.categoriesLeft];
                const categoryDataA =
                    this.rawData["metadata"][this.categoriesLeft];

                const categoryLabelsB =
                    this.rawData["metadata_labels"][this.categoriesRight];
                const categoryDataB =
                    this.rawData["metadata"][this.categoriesRight];
                ...
                for (var i = 0; i < this.expressionData[gene].length; i++) {
                    ...
                    if (!geneExpression[labelA][labelB])
                        geneExpression[labelA][labelB] = [];
                    geneExpression[labelA][labelB].push(
                        this.expressionData[gene][i]
                    );
                }
                ...
                for (const labelA in geneExpression) {
                    ...
                    sortedSubLabels[labelA] = this.sortObjectKeysLocale(
                        sumstat[labelA]
                    );
                }
                ...
                const sortedExpression =
                    this.sortObjectKeysLocale(sortedSubLabels);
            ...
            */
            ...
            function std(arr) {
                const mu = mean(arr);
                return Math.sqrt(
                    arr.reduce((acc, val) => acc + Math.pow(val - mu, 2), 0) /
                        (arr.length - 1)
                );
            }
            ...
            function cohenD(groupA, groupB) {
                ...
                const pooledStd = Math.sqrt(
                    ((groupA.length - 1) * Math.pow(stdA, 2) +
                        (groupB.length - 1) * Math.pow(stdB, 2)) /
            ...
            function dynamicTTest(data) {
                ...
                Object.keys(data).forEach((rowKey) => {
                    const groupData = columnKeys.map(
                        (group) => data[rowKey][group]
                    );
                    results[rowKey] = {};
        ...
        toggleTableDrawer(e = null) {
            const el = !e
                ? document.querySelector(".table-drawer")
                : e.target.parentNode;
            const tableWidth =
                Math.floor(el.getBoundingClientRect().width) - 20;
        ...
        removeGene(gene) {
            ...
            const index = this.expressionStats.findIndex((obj) =>
                obj.hasOwnProperty(gene)
            );
            ...
            if (this.activeGene === gene) {
                ...
                this.activeGene =
                    genes.length > 0 ? genes[genes.length - 1] : "";
        ...
        onSortChanged(ctx) {
            ...
            if (this.aRows.length > 0)
                this.sortedRowsA = this.localSort(this.aRows, sortBy, sortDesc);
            if (this.aRows.length < 1 && this.bRows.length > 0)
                this.sortedRowsB = this.localSort(this.bRows, sortBy, sortDesc);
        ...
        crossTabAnalysis(data) {
            ...
            for (let row in data) {
                ...
                for (let col in data[row]) {
                    ...
                    columnTotals[col] =
                        (columnTotals[col] || 0) + data[row][col];
            ...
            for (let row in data) {
                ...
                for (let col in data[row]) {
                    ...
                    let expectedValue =
                        (rowTotals[row] * columnTotals[col]) / grandTotal;
                    expected[row][col] = expectedValue;

                    let contribution =
                        Math.pow(observedValue - expectedValue, 2) /
                        expectedValue;
            ...
            console.warn(
                "crossTabAnalysis",
                testResult,
                testResult.toString({ decision: true })
            );
        },
        createHeatmapData() {
            ...
            for (const label in statistics) {
                ...
                this.geneNames.forEach((gene) => {
                    ...
                    row[`${gene}_percent`] =
                        statistics[label][gene].pctExpressed;
                });
                tableData.push(row);
            }
            ...
            const fields = [
                ...
                ...this.geneNames.map((gene) => ({
                    key: `${gene}_mean`,
                    label: `${gene} Mean Expression`,
                })),
                ...this.geneNames.map((gene) => ({
                    key: `${gene}_percent`,
        ...
        calculateStatistics() {
            ...
            categoryData.forEach((labelIdx, cellIdx) => {
                ...
                //this.geneNames.forEach(gene => {
                geneExpressionByLabel[label][gene].push(
                    this.expressionData[gene][cellIdx]
                );
                //});
            });
            ...
            for (const label in geneExpressionByLabel) {
                ...
                this.geneNames.forEach((gene) => {
                    ...
                    const mean =
                        dataPoints.reduce((sum, val) => sum + val, 0) / n;
                    const median =
                        (dataPoints[Math.floor((n - 1) / 2)] +
                            dataPoints[Math.ceil((n - 1) / 2)]) /
                        2;
                    ...
                    const pctExpressed =
                        (dataPoints.filter((val) => val > 0).length / n) * 100;

                    const kde = this.kernelDensityEstimator(
                        this.kernelEpanechnikov(7),
                        d3.range(min, max, (max - min) / 100)
                    );
