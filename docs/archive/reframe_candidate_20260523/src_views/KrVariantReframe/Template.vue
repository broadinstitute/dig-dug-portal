    ...
    <div>
        ...
        <div class="container-fluid mdkp-body glens-reframe-page">
            <main class="glens-reframe-shell">
                ...
                <section class="glens-reframe-card glens-reframe-summary-band" aria-label="Variant interpretation summary">
                    ...
                    <div class="glens-reframe-summary-grid">
                        <div class="glens-reframe-summary-item">
                            ...
                            <strong>{{ variant.query.label }}</strong>
                            <small>{{ queryModeLabel }} · {{ variant.query.build }}</small>
                        </div>
                        <div class="glens-reframe-summary-item">
                            <span>Pathogenicity / severity</span>
                            <strong>{{ variant.query.pathogenicity }}</strong>
                            <small>REVEL 0.81 · AlphaMissense 0.94 · exact queried variant evidence</small>
                        </div>
                        <div class="glens-reframe-summary-item">
                            <span>Exact queried-variant carriers</span>
                            <strong>{{ variant.summaryScopes.variant.all }} carriers</strong>
                            <small>{{ affectedCarrierCount }} affected · {{ variant.summaryScopes.variant.proband }} proband · {{ diagnosedCarrierCount }} diagnosed</small>
                        </div>
                        <div class="glens-reframe-summary-item">
                            <span>Top carrier phenotype categories</span>
                            <strong>{{ topCarrierPhenotypeCategories }}</strong>
                            <small>Primary CRDC carrier HPO profile; not a direct variant-context score.</small>
                        </div>
                        <div class="glens-reframe-summary-item">
                            <span>Correlated disease references</span>
                            <strong>{{ topDiseaseReferences }}</strong>
                            <small>Core rare disease reference: Orphanet / HPO / OMIM.</small>
                        </div>
                        <div class="glens-reframe-summary-item">
                            <span>Investigator / cohort concentration</span>
                            <strong>{{ topCohortConcentration }}</strong>
                            <small>Carrier group pattern; separate from phenotype-context match.</small>
                        </div>
                        <div v-if="hasActiveContext" class="glens-reframe-summary-item glens-reframe-summary-item--context">
                            <span>Carrier-context HPO match</span>
                            <strong>{{ carrierContextSummary }}</strong>
                            <small>Active context is compared with carrier HPO profiles, not with the variant itself.</small>
                        </div>
                    </div>
                    <div class="glens-reframe-scope-row">
                        <span class="glens-reframe-scope-tag">Exact queried variant: carrier count and pathogenicity summary</span>
                        <span class="glens-reframe-scope-tag">Same gene: UBE3A recurrence and disease reference</span>
                        <span class="glens-reframe-scope-tag">Nearby variants: locus-window carrier-count context only</span>
                    </div>
                    <p class="glens-reframe-next-step">
                        Inspect next: carrier HPO profile, carrier reference set, and whether carrier groups align with rare disease references.
                    </p>
                </section>
...
export default {
    ...
    computed: {
        ...
        queryModeLabel() {
            return this.activeSummaryLevel === "gene" ? "Gene-level query" : "Exact queried variant";
        },
        affectedCarrierCount() {
            return this.variant.carrierSamples.filter((sample) => sample.affected === "Yes").length;
        },
        diagnosedCarrierCount() {
            return this.variant.carrierSamples.filter((sample) => sample.diagnosed === "Yes").length;
        },
        topCarrierPhenotypeCategories() {
            return this.variant.carrierPhenotypesByCategory
                .slice(0, 2)
                .map((category) => `${category.category} · ${category.count} terms`)
                .join(" | ");
        },
        topDiseaseReferences() {
            return this.variant.diseaseSignals.map((signal) => signal.label).join(" · ");
        },
        topCohortConcentration() {
            const group = this.variant.residualGroups.find((item) => item.name === "Investigator 2") || this.variant.residualGroups[0];
            return group ? `${group.name} · ${group.extreme}` : "No group concentration";
        },
        carrierContextSummary() {
            return this.contextCarrierOverlap.length ? `${this.contextCarrierOverlap.length} carrier HPO terms overlap active context` : "No carrier HPO overlap in visible mock profile";
        },
    },
    methods: {
        sampleHref(sampleId) {
