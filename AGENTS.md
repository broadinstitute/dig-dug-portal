...
<!-- Version: 1.0.13 | Generated: 2026-07-29T22:03:53Z | Hash: c8b826f7237c -->
...
Variant-specific guidance for the `cfde-main` build of dig-dug-portal. Extends ../AGENTS.md; root rules and workflows still apply.

Upstream branch: `cfde-main` of https://github.com/broadinstitute/dig-dug-portal
...
- Branch: `cfde-main`
- Vue config: `configs/vue.config.CFDE.js` (variant-specific page set)
- Differences from default: document only what diverges from parent (entry pages, env vars, deploy target).
- vis-network: `^10.0.2` + `vis-data ^8.0.3` direct — same track as master; no `vue-vis-network` wrapper.
...
```bash
git checkout cfde-main
```

Then follow setup, watch, build, and deploy from ../AGENTS.md.

For local static preview of compiled CFDE output:
