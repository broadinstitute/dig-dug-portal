...
<!-- Version: 1.0.12 | Generated: 2026-05-19T00:50:11Z | Hash: 58b7967237fb -->
...
Variant-specific guidance for the `radiant-main` build of dig-dug-portal. Extends ../AGENTS.md; root rules and workflows still apply.

Upstream branch: `radiant-main` of https://github.com/broadinstitute/dig-dug-portal
...
- Branch: `radiant-main`
- Vue config: `configs/vue.config.RADIANT.js` (variant-specific page set)
...
- Additional visualization: `igv 2.12.6` — IGV.js embedded genome browser (not present in most other tracked branches).
- Uses additional environment variable: `BIOINDEX_HOST_PRIVATE` for specific dataset access (not present in master or most other tracked branches).
...
```bash
git checkout radiant-main
```

Then follow setup, watch, build, and deploy from ../AGENTS.md.

For local static preview of compiled RADIANT output:
